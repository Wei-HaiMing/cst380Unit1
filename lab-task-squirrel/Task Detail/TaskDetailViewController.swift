...
class TaskDetailViewController: UIViewController, UINavigationControllerDelegate {
    ...
    private func presentImagePicker() {
        ...
        config.filter = .images // what do we want to get

        // Request the original file format. Fastest method as it avoids transcoding.
        config.preferredAssetRepresentationMode = .current // what format do we want the files in

        // Only allow 1 image to be selected at a time.
        config.selectionLimit = 1 // how many of media do we want at a time
    ...
    func updateMapView() {
        ...
        guard let imageLocation = task.imageLocation else { return } // guard statement is basically a try except
